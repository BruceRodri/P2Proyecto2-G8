...
app.post("/api/usuarios/register", async (req, res) => {
  try {
    ...
    const insertResult = await pool
      ...
      .query(
        "INSERT INTO usuarios (nombre, correo, contraseña) OUTPUT INSERTED.id VALUES (@nombre, @correo, @contraseña)",
      );

    const newId = insertResult.recordset[0].id;
    res.json({ id: newId, nombre, correo });
  } catch (error) {
  ...
  try {
    ...
    if (NEWSAPI_URL) {
      ...
      if (!NEWSAPI_KEY || NEWSAPI_KEY === "YOUR_NEWSAPI_KEY") {
        return res
          .status(400)
          .json({ error: "API Key de noticias no configurada" });
      }
